...
static void odroid_reboot(char str, const char *cmd)
{
	...
	writel(0x0, S5P_INFORM4);		/* Reset reboot count */
        ...
        gpio_direction_output(g_pdata->power_gpio, 0);
        mdelay(150);
        gpio_direction_output(g_pdata->power_gpio, 1);
...
static int odroid_reboot_remove(struct platform_device *pdev)
{
	return 0;
}
