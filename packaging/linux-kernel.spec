...
#tar cpsf - . --one-file-system | tar xfC - %{buildroot}/usr/src/linux-kernel-sources-%{version}-%{abiver}

make ARCH=arm EXTRAVERSION="-%{abiver}" O=%{buildroot}/usr/src/linux-kernel-build-%{version}-%{abiver} tizen_defconfig
make ARCH=arm EXTRAVERSION="-%{abiver}" O=%{buildroot}/usr/src/linux-kernel-build-%{version}-%{abiver} %{?_smp_mflags}
...
ln -sf ../linux-kernel-sources-%{version}-%{abiver} %{buildroot}/usr/src/linux-kernel-build-%{version}-%{abiver}/source
