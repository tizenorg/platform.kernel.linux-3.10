...
static int odroid_audio_probe(struct platform_device *pdev)
{
	...
	ret = snd_soc_of_parse_card_name(card, "exynos4412,model");
	if (ret)
		goto err;

	pr_err("Card name is %s\n", card->name);

	odroid_dai[0].codec_name = NULL;
	odroid_dai[0].codec_of_node = of_parse_phandle(np,
	...
	odroid_dai[0].cpu_name = NULL;
	odroid_dai[0].cpu_of_node = of_parse_phandle(np,
						"samsung,i2s-controller", 0);
	if (!odroid_dai[0].cpu_of_node) {
		dev_err(&pdev->dev,
	...
	odroid_dai[0].platform_of_node = odroid_dai[0].cpu_of_node;
...
static const struct of_device_id odroid_audio_of_match[] = {
	{ .compatible = "samsung,exynos4-odroid", },
	{},
};
MODULE_DEVICE_TABLE(of, odroid_audio_of_match);


static struct platform_driver odroid_driver = {
	.driver		= {
		.name	= "odroid-audio",
		.owner	= THIS_MODULE,
...
module_platform_driver(odroid_driver);

MODULE_AUTHOR("zhen1.chen@samsung.com");
MODULE_DESCRIPTION("ALSA SoC For Odroid-X2");
MODULE_LICENSE("GPL");
