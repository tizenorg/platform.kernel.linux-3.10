				...
				struct snd_pcm_hw_params *params)
{
	...
	if (epll_freq != clk_get_rate(clk_pll)) {
		ret = clk_set_rate(clk_pll, epll_freq);
		if (ret < 0)
			return ret;
		pr_info("%s: fout_epll: %ld (%ld) Hz\n", __func__,
			clk_get_rate(clk_pll), epll_freq);
	}
