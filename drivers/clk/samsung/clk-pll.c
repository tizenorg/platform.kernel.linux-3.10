...
static inline void samsung_pll35xx_get_mps(struct samsung_clk_pll35xx *pll,
					   u32 *m, u32 *p, u32 *s)
{
	u32 pll_con = __raw_readl(pll->base_reg + pll->con_offset);
				...
				unsigned long prate)
{
	...
	if (p == p_cur && m == m_cur) {
		tmp = __raw_readl(pll->base_reg + pll->con_offset);
		tmp &= ~(PLL35XX_SDIV_MASK << PLL35XX_SDIV_SHIFT);
		tmp |= s << PLL35XX_SDIV_SHIFT;
		__raw_writel(tmp, (u32 *)(pll->base_reg	+ pll->con_offset));
...
samsung_clk_register_pll35xx(const char *name,
			     ...
			     struct pll_pms *pms, unsigned int con_offset)
{
	...
	pll->pms = pms;
	pll->con_offset = PLL35XX_PLL_CON0 + con_offset;
