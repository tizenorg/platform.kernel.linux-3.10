...
static void odroid_usbotg_work(struct work_struct *work)
{
	int state;
	struct odroid_usbotg_data	*data =
		container_of(to_delayed_work(work), struct odroid_usbotg_data,
			     work);

	if (data->otg_id_gpio)
		state = (gpio_get_value(data->vbus_det_gpio) << 1) |
			gpio_get_value(data->otg_id_gpio);
	else
		state = (gpio_get_value(data->vbus_det_gpio) << 1) | 1;

	extcon_set_cable_state_(data->edev, EXTCON_CABLE_USB_HOST, (state == 0));
	extcon_set_cable_state_(data->edev, EXTCON_CABLE_USB, (state == 3));
}
				    ...
				    struct odroid_usbotg_data *extcon_data)
{
	...
	if (of_property_read_u32(np, "debounce", &val) != 0)
		val = 50;
	extcon_data->debounce_jiffies = msecs_to_jiffies(val);
...
static int odroid_usbotg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&extcon_data->work, odroid_usbotg_work);
	...
	odroid_usbotg_work(&extcon_data->work.work);
